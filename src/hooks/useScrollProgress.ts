...
export function useScrollProgress(
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      const backScale = 1 + progress * 0.1;
      ...
      set('--shade-bottom-alpha', String(blurActive * 0.5));
      set('--shade-z', blurActive > 0.02 ? '5' : '0');

      // ── 7. Story Panel 1 (The pause before you pay) ──────────
      const panel2Opacity = Math.max(0, Math.min(frame2Enter * 1.8, 1) * (1 - frame2Exit * 1.8));
      ...
      const panel3Opacity = Math.max(0, Math.min(frame3Enter * 1.8, 1) * (1 - frame3Exit * 1.8));
